 ...
 *  MOOS message will be transmiteed through AquaNET
 */
#include <ifaddrs.h>
#include <arpa/inet.h>
#include <netdb.h>
...
//		}
				...
				q++)
		{
			...
			{
				//std::cout << "find route  for IO Configuration" << std::endl;
				//this looks like a long hand configuration
				ProcessIOConfigurationString(*q,true);
			}
			else
			{
				//std::cout << "find route  for short Configuration" << std::endl;
				...
				double frequency)
{
    ...
    /* 
		not wildcard route in this code
		Shuai Dong
	*/
	if(trimed_src_name.find_last_of("*?:")==std::string::npos)
	{
		std::cout << "not multicast routing " << std::endl;
		Route route;
...
bool Share::Impl::DoRegistrations()
{
	...
	{
		std::cout << "wild card routing table is running " << std::endl;
...
bool Share::Impl::PrintSocketMap()
{
	...
	std::cout << "Shuai Dong print the socketMap" << std::endl;
...
{
	std::cout << "WildcardMatch" << std::endl;
...
// 	std::cout << "Share::Impl::ApplyWildcardRoutes( CMOOSMsg& msg)" << std::endl;
// 	WildcardRouteMap::iterator g;
// 	for(g=wildcard_routing_table_.begin();g!=wildcard_routing_table_.end();g++)
// 	{
...
{
	std::cout << "GetAddressFromChannelAlias" << std::endl;
...
/*
	adding listener.cpp function here
	DATE: 07/06/2021
	Author: Shuai Dong
*/
